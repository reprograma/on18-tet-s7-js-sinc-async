...
async function resolverPromise() {
    try {
        ...
        const resultados = await Promise.all([ //recebe uma array. nesse caso tanto o pegar endereço quanto pegar pedidos vão acontecer ao mesmo tempo
            pegarEndereco(usuaria.id),         
            pegarPedidos(usuaria.id)
        ])
        const endereco = resultados[0] //como é a primeira requisição usamos o 0, historico é a segunda e por isso 1. lembrar como funciona array.
